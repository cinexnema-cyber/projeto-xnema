...
export default function CreatorPortal() {
  ...
  const uploadedVideos = [
    {
      ...
      thumbnail: "https://cdn.builder.io/api/v1/image/assets%2Ff280dc7f1a3b442bb1f2a4e0b57c6521%2F53ce9d12d034482db26dcf63073a2cfe?format=webp&width=200"
    },
    {
      id: 2,
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "Publicado": return <CheckCircle className="w-4 h-4 text-green-500" />;
        ...
        <div className="container mx-auto px-4">
          ...
          {loading ? (
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-xnema-orange">{creatorStats.totalVideos}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-xnema-orange">{creatorStats.totalViews.toLocaleString()}</div>
                <p className="text-xs text-muted-foreground">
                  +12.4% vs mês passado
                </p>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-xnema-orange">R$ {creatorStats.totalRevenue.toFixed(2)}</div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card className="cursor-pointer hover:bg-xnema-surface transition-colors" asChild>
              <Link to="/creator-payments">
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-yellow-500">R$ {creatorStats.pendingRevenue.toFixed(2)}</div>
                  <p className="text-xs text-muted-foreground">
            ...
            <TabsContent value="create" className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Crie conteúdo que aparece automaticamente para os assinantes</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <h4 className="font-semibold text-foreground">Link do Vídeo</h4>
                      <input
                        type="url"
                      ...
                      <h4 className="font-semibold text-foreground">Categoria</h4>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <h4 className="font-semibold text-foreground mb-2">Temporada</h4>
                          <input type="number" min="1" max="7" defaultValue="1" className="flex h-10 w-full rounded-md border border-xnema-border bg-background px-3 py-2 text-sm text-foreground" />
                        </div>
                        <div>
                          <h4 className="font-semibold text-foreground mb-2">Episódio</h4>
                          <input type="number" min="1" max="12" defaultValue="1" className="flex h-10 w-full rounded-md border border-xnema-border bg-background px-3 py-2 text-sm text-foreground" />
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-blue-500/20 to-cyan-500/20 border border-blue-500/30 rounded-lg p-4">
                    <h4 className="font-semibold text-foreground mb-2">🤖 Automação Inteligente</h4>
                    <ul className="text-sm text-muted-foreground space-y-1">
                      <li>• O conteúdo aparece automaticamente no catálogo</li>
                      <li>• Thumbnail é extraída automaticamente do vídeo</li>
                      <li>• Assinantes são notificados sobre novo conteúdo</li>
                      <li>• Analytics são gerados em tempo real</li>
                      <li>• Receita é calculada e creditada automaticamente</li>
                    </ul>
                  </div>

                  <div className="flex space-x-4">
                    <Button className="bg-xnema-orange hover:bg-xnema-orange/90 text-black" asChild>
            ...
            <TabsContent value="upload" className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Faça upload dos seus vídeos para a série Between Heaven and Hell</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  {/* Video Upload */}
                  <div className="space-y-4">
                    <label className="text-sm font-medium text-foreground">Vídeo Principal</label>
                    <div className="border-2 border-dashed border-xnema-border rounded-lg p-8 text-center hover:border-xnema-orange transition-colors">
                      ...
                      <p className="text-foreground font-medium mb-2">Clique para fazer upload ou arraste o arquivo</p>
                      <p className="text-sm text-muted-foreground mb-4">MP4, MOV, AVI até 5GB</p>
                  ...
                  <div className="grid gap-4">
                    <div className="grid gap-2">
                      <label className="text-sm font-medium text-foreground">Título do Episódio</label>
                      <input 
                        type="text" 
                    ...
                    <div className="grid gap-2">
                      <label className="text-sm font-medium text-foreground">Link do Vídeo (YouTube/Vimeo)</label>
                      <input 
                        type="url" 
                      ...
                      />
                      <p className="text-xs text-muted-foreground">
                        Cole o link direto do vídeo hospedado no YouTube, Vimeo ou outro serviço
                      </p>
                    </div>
                  </div>
            ...
            <TabsContent value="videos" className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Gerencie todo o seu conteúdo publicado</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {uploadedVideos.map((video) => (
                      <div key={video.id} className="flex items-center space-x-4 p-4 bg-xnema-surface rounded-lg">
                        <img
                          src={video.thumbnail}
                          alt={video.title}
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground">
                            {video.series} • T{video.season}E{video.episode} • {video.uploadDate}
                          </p>
                        </div>
                        
                        <div className="text-right">
                          <div className="text-sm font-semibold text-foreground">{video.views.toLocaleString()} views</div>
                          <div className="text-sm text-xnema-orange">R$ {video.revenue.toFixed(2)}</div>
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-6">
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <h3 className="text-lg font-semibold text-foreground">Período Promocional</h3>
                        <div className="bg-gradient-to-r from-green-500/20 to-green-600/20 border border-green-500/30 rounded-lg p-4">
                          <div className="flex items-center space-x-2 mb-2">
                            ...
                            <span className="font-semibold text-foreground">100% da receita para você</span>
                          </div>
                          <p className="text-sm text-muted-foreground">
                            Você está no período promocional de 3 meses. Toda receita é sua!
                          </p>
                          <div className="mt-3">
                            ...
                            <div className="w-full bg-muted rounded-full h-2">
                              <div className="bg-green-500 h-2 rounded-full" style={{ width: '66%' }} />
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-center p-3 bg-xnema-surface rounded">
                            <span className="text-foreground">Dezembro 2024</span>
                            <span className="font-semibold text-xnema-orange">R$ 890.30</span>
                          </div>
                          <div className="flex justify-between items-center p-3 bg-xnema-surface rounded">
                            <span className="text-foreground">Janeiro 2025</span>
                            <span className="font-semibold text-muted-foreground">R$ 245.80 (pendente)</span>
                          </div>
                        </div>
                      </div>
                    </div>
