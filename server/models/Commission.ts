...
CommissionSchema.methods.marcarComoPago = function(metodoPagamento: string, referencia?: string) {
  this.status = 'pago';
...
CommissionSchema.statics.calcularSaldoDisponivel = function(criadorId: string) {
...
CommissionSchema.post('save', async function(doc) {
  if (doc.status === 'disponivel' || doc.status === 'pago') {
    try {
      ...
      const saldoResult = await Commission.calcularSaldoDisponivel(doc.id_criador.toString());
      const saldoDisponivel = saldoResult[0]?.total || 0;
      
      // Calcula total ganho
      const totalResult = await Commission.calcularTotalGanho(doc.id_criador.toString());
