  ...
  try {
    ...
    const ganhosPorDia = await Commission.aggregate([
      {
        $match: {
          id_criador: new mongoose.Types.ObjectId(creatorId),
          data_geracao: { $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) }
        }
      },
      {
        $group: {
    ...
    res.json({
      ...
      periodo: {
        inicio: dataInicial,
        fim: new Date(),
        dias: Math.ceil((Date.now() - dataInicial.getTime()) / (1000 * 60 * 60 * 24))
      },
    ...
    const novosAssinantes = await Subscription.aggregate([
      ...
      },
      {
        $match: {
          ...
          data_pagamento: { $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) },
  ...
  try {
    ...
    try {
      ...
      const comissoesDisponiveis = await Commission.find({
        id_criador: creatorId,
        status: 'disponivel'
      }).sort({ data_geracao: 1 }).session(session);
...
router.get('/videos/:creatorId', async (req: Request, res: Response) => {
  try {
    ...
    const videos = await Video.find(filtro)
      ...
      .skip(skip)
      .limit(Number(limit))
      .select('titulo descricao thumbnail status categoria visualizacoes curtidas data_upload tipo');
...
router.put('/profile/:creatorId', async (req: Request, res: Response) => {
  try {
    ...
    if (comissaoPercentual && comissaoPercentual >= 0 && comissaoPercentual <= 100) {
