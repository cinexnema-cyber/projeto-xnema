...
const MERCADO_PAGO_PLANS = {
  basic: {
    ...
    name: 'Plano Básico',
    price: 9.90,
    checkoutUrl: 'https://www.mercadopago.com.br/subscriptions/checkout?preapproval_plan_id=basic_plan_id'
  },
  premium: {
    id: '68bba11ef1bc4daa9275b1ccfd668120',
    name: 'Plano Premium',
    price: 19.90,
    checkoutUrl: 'https://www.mercadopago.com.br/subscriptions/checkout?preapproval_plan_id=68bba11ef1bc4daa9275b1ccfd668120'
  },
  premium_plus: {
    id: 'premium_plus_plan_id',
    name: 'Plano Premium Plus',
    price: 29.90,
    checkoutUrl: 'https://www.mercadopago.com.br/subscriptions/checkout?preapproval_plan_id=premium_plus_plan_id'
  }
};
...
export const createSubscription = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const planData = MERCADO_PAGO_PLANS[plan as keyof typeof MERCADO_PAGO_PLANS];
...
export const cancelSubscription = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    console.log(`Subscription cancelled for user ${user.email}. Reason: ${reason || 'Not provided'}`);
...
export const getSubscriptionStatus = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const isActive = subscription?.status === 'active' && 
                    (!subscription.nextBilling || new Date(subscription.nextBilling) > new Date());
