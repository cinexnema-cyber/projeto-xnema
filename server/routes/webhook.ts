...
async function handleInvoicePaymentSucceeded(invoice: Stripe.Invoice) {
  ...
  if (creatorId && valorPago > 0) {
    ...
    console.log(`💸 Comissão criada: R$ ${(comissaoCriador / 100).toFixed(2)} para criador ${creatorId}`);
  }
  ...
  try {
    if (mongoose.connection.readyState === 1) {
      ...
      const newSubscription = new Subscription({
        ...
        plano: subscription.items.data[0]?.price?.recurring?.interval === 'year' ? 'yearly' : 'monthly',
...
async function handleSubscriptionCreated(subscription: Stripe.Subscription) {
  ...
  console.log(`👤 Cliente: ${customerId}, Criador: ${creatorId}, Usuário: ${userId}`);
}
...
router.get('/stats', (req: Request, res: Response) => {
  res.json({
    ...
    estatisticas: {
      ...
      total_revenue: db.subscriptions.reduce((sum, s) => sum + (s.valor_pago || 0), 0),
      total_commissions: db.commissions.reduce((sum, c) => sum + (c.valor_comissao || 0), 0)
    },
