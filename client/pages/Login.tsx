...
export default function Login() {
  ...
  const handleCreatorLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (creatorForm.email === "cinexnema@gmail.com" && creatorForm.password === "I30C77T$Ii") {
        const result = await login({
        ...
        if (result.success) {
          ...
          setErrorMessage(result.message || "Erro ao fazer login de admin");
        }
      } else {
  ...
  const handlePaymentRedirect = async () => {
    // First, register user before payment
    if (!subscriberForm.email || !subscriberForm.password) {
      setErrorMessage("Por favor, preencha email e senha antes de prosseguir");
      return;
    }

    try {
      const { register } = useAuth();
      const preRegisterResult = await fetch("/api/subscription/pre-register", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          email: subscriberForm.email,
          password: subscriberForm.password,
          name: subscriberForm.email.split('@')[0]
        }),
      });

      const preRegData = await preRegisterResult.json();

      if (!preRegData.success) {
        setErrorMessage(preRegData.message || "Erro ao criar conta");
        return;
      }
      ...
      setErrorMessage("Erro ao processar cadastro e pagamento");
    }
  };
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <Tabs defaultValue="subscriber" className="w-full">
              <TabsList className={`grid w-full ${showAdminAccess ? 'grid-cols-3' : 'grid-cols-2'} mb-8 h-auto`}>
                <TabsTrigger value="subscriber" className="text-sm sm:text-lg py-3 px-2 sm:px-4 flex-1 min-h-[50px]">
                  <Crown className="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2" />
                  <span className="truncate">Assinante</span>
                </TabsTrigger>
                <TabsTrigger value="creator" className="text-sm sm:text-lg py-3 px-2 sm:px-4 flex-1 min-h-[50px]">
                  <Video className="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2" />
                  <span className="truncate">Criador</span>
                </TabsTrigger>
                {showAdminAccess && (
                  <TabsTrigger value="admin" className="text-sm sm:text-lg py-3 px-2 sm:px-4 flex-1 min-h-[50px]">
