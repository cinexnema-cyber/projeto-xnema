  ...
  const fetchUsers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setUsers(data.users);
        setPendingCreators(data.users.filter((u: any) => u.role === 'creator' && u.profile?.status === 'pending'));
      }
    } catch (error) {
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-500">R$ {stats.monthlyRevenue.toFixed(2)}</div>
                <p className="text-xs text-muted-foreground">
                  Este mês
                </p>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-blue-500">{stats.contentViews.toLocaleString()}</div>
                <p className="text-xs text-muted-foreground">
                  Total de views
                </p>
              </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {users.map((user) => (
                      <div key={user.id} className="flex items-center justify-between p-4 border border-xnema-border rounded-lg">
                        <div className="flex items-center space-x-4">
                          <div className="w-10 h-10 bg-gradient-to-br from-xnema-orange to-xnema-purple rounded-full flex items-center justify-center">
                            {user.role === 'admin' ? <Settings className="w-5 h-5 text-black" /> :
                             user.role === 'creator' ? <Video className="w-5 h-5 text-black" /> :
                        ...
                        <div className="flex items-center space-x-2">
                          <Badge variant={
                            ...
                            {user.role === 'admin' ? 'Admin' :
                             user.role === 'creator' ? 'Criador' : 'Assinante'}
                          </Badge>
                          <p className="text-xs text-muted-foreground">
                  ...
                  {pendingCreators.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {pendingCreators.map((creator) => (
                        <div key={creator.id} className="p-6 border border-xnema-border rounded-lg">
                          <div className="flex items-start justify-between mb-4">
                            <div>
                              <h4 className="text-lg font-semibold text-foreground">{creator.name}</h4>
                          ...
                          {creator.profile.bio && (
                            <div className="mb-4">
                              <h5 className="font-medium text-foreground mb-2">Bio:</h5>
                              <p className="text-sm text-muted-foreground">{creator.profile.bio}</p>
                            </div>
                          )}
                          
                          {creator.profile.portfolio && (
                            <div className="mb-4">
                              <h5 className="font-medium text-foreground mb-2">Portfólio:</h5>
                              <a 
                          ...
                          <div className="flex space-x-2">
                            <Button 
                              ...
                              onClick={() => handleCreatorApproval(creator.id, true)}
                            >
                            ...
                            <Button 
                              ...
                              onClick={() => handleCreatorApproval(creator.id, false)}
                            >
                ...
                <CardContent>
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground mb-4">
                      Esta funcionalidade permitirá revisar e aprovar vídeos enviados pelos criadores.
                    </p>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground mb-4">
                      Relatórios detalhados sobre usuários, receita e engajamento.
                    </p>
                    <Button variant="outline">
                      Em Desenvolvimento
                    </Button>
