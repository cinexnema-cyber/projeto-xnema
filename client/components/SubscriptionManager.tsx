          ...
          <CardContent className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground">
                  R$ {subscription.plan === 'premium' ? '19,90' : '9,90'}/mês
                </p>
              </div>
              <Badge variant={isActive ? "default" : "secondary"}>
                {subscription.status === 'active' ? 'Ativo' :
                 subscription.status === 'pending' ? 'Pendente' :
                 subscription.status === 'cancelled' ? 'Cancelado' : 'Inativo'}
              </Badge>
            </div>
            ...
            {subscription.status === 'pending' && (
              <Alert>
                ...
                <AlertDescription>
                  Pagamento em processamento. Você será notificado quando aprovado.
                </AlertDescription>
              </Alert>
            )}
            ...
            {subscription.status === 'cancelled' && (
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  Assinatura cancelada. Você manterá acesso até {subscription.nextBilling ? new Date(subscription.nextBilling).toLocaleDateString('pt-BR') : 'o final do período'}.
                </AlertDescription>
              </Alert>
            )}
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              {subscription.nextBilling && (
                <div>
                  ...
                  <p className="font-medium">{new Date(subscription.nextBilling).toLocaleDateString('pt-BR')}</p>
                </div>
              )}
            </div>
            ...
            {isActive && subscription.status !== 'cancelled' && (
              <div className="pt-4 border-t border-xnema-border">
                <h4 className="font-medium text-foreground mb-2">Cancelar Assinatura</h4>
                <p className="text-sm text-muted-foreground mb-4">
                  Você pode cancelar sua assinatura a qualquer momento. O acesso será mantido até o final do período pago.
                </p>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid md:grid-cols-3 gap-6">
            {plans.map((plan) => (
              <Card key={plan.id} className={plan.id === 'premium' ? 'border-xnema-orange' : ''}>
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    {plan.name}
                    {plan.id === 'premium' && (
                      <Badge className="bg-xnema-orange text-black">Recomendado</Badge>
                    )}
                  </CardTitle>
                  <CardDescription>
