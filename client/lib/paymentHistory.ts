...
export class PaymentHistoryService {
   ...
   */
  static async addPaymentRecord(paymentData: Omit<PaymentRecord, 'id' | 'created_at' | 'updated_at'>): Promise<{ success: boolean; error?: string; data?: PaymentRecord }> {
    try {
   ...
   */
  static async getUserPaymentHistory(userId: string): Promise<{ success: boolean; error?: string; data?: PaymentRecord[] }> {
    try {
      const { data, error } = await supabase
        .from('payment_history')
        .select('*')
   ...
   */
  static async updatePaymentStatus(paymentId: string, status: PaymentRecord['status'], metadata?: PaymentRecord['metadata']): Promise<{ success: boolean; error?: string }> {
    try {
   ...
   */
  static async getUserPaymentSummary(userId: string): Promise<{ success: boolean; error?: string; data?: PaymentSummary }> {
    try {
   ...
   */
  static async createSamplePayments(userId: string, userEmail: string): Promise<{ success: boolean; error?: string }> {
    const samplePayments: Omit<PaymentRecord, 'id' | 'created_at' | 'updated_at'>[] = [
      {
        user_id: userId,
        ...
        plan_type: 'monthly',
        status: 'completed',
        payment_date: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString(), // 30 dias atrás
      ...
      },
      {
        ...
        subscription_end_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(), // 30 dias no futuro
        metadata: {
      ...
      },
      {
        ...
        plan_type: 'yearly',
        status: 'pending',
        payment_date: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), // 2 dias atrás
        metadata: {
